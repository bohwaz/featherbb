...
class Index
{
    ...
    public function get_forum_actions()
    {
        ...
        if (!$this->user->is_guest) {
            $forum_actions[] = '<a href="'.$this->feather->pathFor('markRead').'">'.__('Mark all as read').'</a>';
        }
    ...
    public function print_categories_forums()
    {
        ...
        foreach ($query as $cur_forum) {
            ...
            if (isset($new_topics[$cur_forum->fid])) {
                ...
                $forum_field_new = '<span class="newtext">[ <a href="'.$this->feather->pathFor('quickSearch', ['show' => '?action=show_new&amp;fid='.$cur_forum->fid]).'">'.__('New posts').'</a> ]</span>';
            ...
            if ($cur_forum->redirect_url != '') {
                ...
                $cur_forum->forum_field = '<h3><a href="'.$this->feather->pathFor('Forum', ['id' => $cur_forum->fid, 'name' => $forum_name]).'">'.Utils::escape($cur_forum->forum_name).'</a>'.(!empty($forum_field_new) ? ' '.$forum_field_new : '').'</h3>';
            ...
            if ($cur_forum->last_post != '') {
                $cur_forum->last_post_formatted = '<a href="'.$this->feather->pathFor('viewPost', ['pid' => $cur_forum->last_post_id]).'#p'.$cur_forum->last_post_id.'">'.$this->feather->utils->format_time($cur_forum->last_post).'</a> <span class="byuser">'.__('by').' '.Utils::escape($cur_forum->last_poster).'</span>';
            ...
            if ($cur_forum->moderators != '') {
                ...
                foreach ($mods_array as $mod_username => $mod_id) {
                    if ($this->user->g_view_users == '1') {
                        $moderators[] = '<a href="'.$this->feather->pathFor('userProfile', ['id' => $mod_id]).'">'.Utils::escape($mod_username).'</a>';
                    } else {
    ...
    public function collect_stats()
    {
        ...
        if ($this->user->g_view_users == '1') {
            $stats['newest_user'] = '<a href="'.$this->feather->pathFor('userProfile', ['id' => $stats['last_user']['id']]).'">'.Utils::escape($stats['last_user']['username']).'</a>';
        } else {
    ...
    public function fetch_users_online()
    {
        ...
        foreach($query as $user_online) {
            if ($user_online->user_id > 1) {
                if ($this->user->g_view_users == '1') {
                    $online['users'][] = "\n\t\t\t\t".'<dd><a href="'.$this->feather->pathFor('userProfile', ['id' => $user_online->user_id]).'">'.Utils::escape($user_online->ident).'</a>';
                } else {
